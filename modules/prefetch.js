(function(scope){
	...
	scope[prefetchID]={
		fireEvent:function(){
			...
			handlePageload();
			if(store.static){
				store.prefetched[store.target.href]=true;
			}
	...
	function registerLinkListeners(element){
		element.addEventListener("mouseenter",linkMouseEnterListener);
		element.addEventListener("mouseout",linkMouseOutListener);
		element.addEventListener("mousedown",linkMouseDownListener);
		element.addEventListener("mouseup",linkMouseUpListener);
		element.addEventListener("click",linkMouseClickListener);
	}
	function handlePageload(){
		//console.log({context:"handlePageload",store:store});
		if((store.loaded||store.prefetched[store.target.href])&&store.static&&store.mouse){
			navigateToTarget();
		}
	}
	function navigateToTarget(){
		scope.location.href = store.target.href;
	}
}(window));
